'use client'
import { motion } from "framer-motion";
import {
  TrendingUp,
  Filter,
  MousePointerClick,
  ChartNoAxesColumn,
  CalendarX2,
  EyeOff,
  Repeat,
  AlertTriangle,
} from "lucide-react";

const problems = [
  { icon: TrendingUp, text: "Rising advertising costs with low ROI" },
  { icon: Filter, text: "Poor-quality leads that never convert" },
  { icon: MousePointerClick, text: "Websites getting traffic but no inquiries" },
  { icon: ChartNoAxesColumn, text: "Low conversion rates across campaigns" },
  { icon: CalendarX2, text: "Inconsistent lead flow month after month" },
  { icon: EyeOff, text: "Weak visibility compared to competitors" },
  { icon: Repeat, text: "No customer retention strategy" },
  { icon: AlertTriangle, text: "Agencies that overpromise and underdeliver" },
];

export const Problems = () => {
  return (
    <section
      id="problems"
      data-testid="problems-grid"
      className="relative py-16 bg-secondary/40 border-y border-border"
    >
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        <div className="max-w-3xl mb-12">
          <p className="text-xs uppercase tracking-[0.3em] text-brand font-semibold mb-5">
            ▸ The Problem
          </p>
          <h2
            data-testid="problems-headline"
            className="font-display font-bold uppercase text-balance leading-[0.95] tracking-tight text-[clamp(2rem,5vw,4.25rem)] mb-6"
          >
            Why Most Businesses Struggle To Grow Online
          </h2>
          <p className="text-base md:text-lg text-muted-foreground max-w-2xl">
            Businesses today are spending more on marketing than ever before.
            Yet many still face the same persistent problems holding them back
            from real growth.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-l border-t border-border">
          {problems.map((p, i) => {
            const Icon = p.icon;
            return (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 18 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-50px" }}
                transition={{ duration: 0.5, delay: i * 0.04 }}
                data-testid={`problem-card-${i}`}
                className="group relative p-7 md:p-8 border-r border-b border-border bg-background hover:bg-card transition-colors"
              >
                <div className="absolute top-0 left-0 h-0.5 w-0 bg-brand transition-all duration-300 group-hover:w-full" />
                <div className="w-10 h-10 rounded-lg border border-border bg-secondary/60 grid place-items-center mb-5 group-hover:border-brand group-hover:text-brand transition-colors">
                  <Icon size={18} />
                </div>
                <p className="text-sm md:text-base font-medium text-foreground leading-snug">
                  {p.text}
                </p>
                <div className="mt-5 flex items-center gap-2 text-[11px] uppercase tracking-wider text-muted-foreground">
                  <span className="text-brand">×</span>
                  Common pain
                </div>
              </motion.div>
            );
          })}
        </div>

        <p
          data-testid="problems-footer"
          className="mt-12 text-center text-lg md:text-xl text-foreground font-semibold"
        >
          Growth shouldn't feel unpredictable.{" "}
          <span className="text-brand">We make it a system.</span>
        </p>
      </div>
    </section>
  );
};
