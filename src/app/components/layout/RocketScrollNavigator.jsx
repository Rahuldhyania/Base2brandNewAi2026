 ...
 * Right-side vertical scroll-spy rail.
 *
 * Behaviour requested by stakeholder (REVERSED FLOW):
 *  - Rocket points upward (ascending out of space) along an orange progress line.
 *  - Sequence circles run bottom → top, one per section (first section at
 *    the bottom, last section at the top) so the rocket visually climbs as
 ...
 * Visible only on lg+ screens to keep mobile clean.
 *
 * `sections` is passed in as a prop (array of { id, label }) so this
 * component can be reused across different pages with different section
 * lists. DEFAULT_SECTIONS is exported as a fallback / reference shape.
 */
...
export function RocketScrollNavigator({ sections }) {
  const containerRef = useRef(null);
  const { scrollYProgress } = useScroll();
  const smoothProgress = useSpring(scrollYProgress, {
    stiffness: 120,
  ...
  // Progress line now fills from the BOTTOM upward.
  const progressHeight = useTransform(smoothProgress, (v) => `${v * 100}%`);
  // Rocket now starts at the BOTTOM (v=0) and climbs to the TOP (v=1).
  const rocketY = useTransform(smoothProgress, (v) =>
    Math.round((1 - v) * (railHeight - 28))
  );
  const progressPercent = useTransform(smoothProgress, (v) =>
    `${Math.round(v * 100).toString().padStart(2, "0")}%`
  );
  ...
  // Dots are now laid out bottom → top: first section sits at the bottom,
  // last section sits at the top, matching the rocket's upward climb.
  const dots = useMemo(
    () =>
      sections.map((s, i) => ({
        ...s,
        topPct: 100 - (i / (sections.length - 1)) * 100,
      })),
    [sections]
  );
      ...
      <div className="relative flex items-start gap-3 pointer-events-auto">
        ...
        >
                ...
                <span
                  ...
                  style={{
                    ...
                    background: reached
                      ...
                      : "white",
                    boxShadow: active
                      ? "0 0 0 3px var(--b2b-primary)/30, 0 0 14px var(--b2b-primary)/20"
                      : "none",
                  }}
                />
          ...
          <m.div
            ...
            data-testid="rocket-nav-rocket"
          >
            <div className="relative -translate-y-1/2 left-3 rotate-349">
              <span
                className="absolute left-1/2 -translate-x-1/2 bottom-3 block w-px h-5 bg-(--b2b-primary)/60 blur-[1px]"
                aria-hidden
              />
              <svg
                viewBox="540 -4 32 60"
